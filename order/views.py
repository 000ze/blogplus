    ...
    if res == "zx":
        if request.method == "POST":
            ...
            if new_title:
                ...
                # 创建产品对象,自动提交
                models.Product.objects.create(title=new_title, merchant_id=new_merchant_id,price=new_price)
                # 返回到产品列表页
                return redirect("/product_list/")
            else:
                # 取到所有的商家
...
def edit_product(request):
    if request.method == "POST":
        ...
        edit_product_obj.price=new_price
        # 将修改提交到数据库
...
def user_list(request):
    ...
    all_price=models.User.objects.all().annotate(price_sum=Sum("product__price"))
    return render(request, "user_list.html", {"user_list": all_user, 'products': ret,'all_price':all_price})
